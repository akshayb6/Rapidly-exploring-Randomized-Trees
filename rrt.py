...
def createPolyPatch(polygon, color):
    verts = []
    codes= []
    for v in range(0, len(polygon)):
        xy = polygon[v]
...
def CollissionFreeLine((px,py),(x_min,y_min),robot,obstacles):
    ...
    while x!=x_min:
        #print x
        y=m*x+c
        if isCollisionFree(robot,(x,y),obstacles):
            x=x+x_iter
            continue
        else:
            return False
...
def pointCalculator(px,py,pi,points,adjListMap):
    min_dist=1000
    x_min=-5
    y_min=-5
    i_min=-1
...
def pointCalculator2(px,py,pi,points,adjListMap,robot,obstacles):
    min_dist=1000
    x_min=-5
    y_min=-5
    i_min=-1
...
def growSimpleRRT(points):
    ...
    for i in points.keys():
        if not (adjListMap[i]):
            px=points[i][0]
            py=points[i][1]
            newPoints,adjListMap=pointCalculator(px,py,i,newPoints,adjListMap)
...
def displayRRTandPath(points, tree, path, robotStart='None', robotGoal='None', polygons='None'):
    ...
    if(robotStart!='None' and robotGoal!='None' and polygons!='None'):
        patch = createPolyPatch(robotStart, 'green')
        ax.add_patch(patch)    
        patch = createPolyPatch(robotGoal, 'red')
        ax.add_patch(patch)    
        for p in range(0, len(polygons)):
            patch = createPolyPatch(polygons[p], 'gray')
            ax.add_patch(patch)   
...
def RRT(robot, obstacles, startPoint, goalPoint):
    ...
    path = []
    # Your code goes here.
    #print isCollisionFree(robot,(0,0.2),obstacles)
    j=100
    while j<5500:
        points[1]=startPoint
        ...
        for i in points.keys():
            if not (adjListMap[i]):
                px=points[i][0]
                py=points[i][1]
                newPoints,adjListMap=pointCalculator2(px,py,i,newPoints,adjListMap,robot,obstacles)
